...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, max_len, device, msg, global_step,
                   num_examples=2):
    model.eval()
    count = 0
    ...
    with torch.no_grad():
        for batch in validation_ds:
            ...
            msg('-' * console_width)
            msg(f"{f'SOURCE: ':>12}{source_text}")
            msg(f"{f'TARGET: ':>12}{target_text}")
            msg(f"{f'PREDICTED: ':>12}{model_out_text}")
